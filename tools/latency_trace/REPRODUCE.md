...
两个轴都能缩放，但含义不同：**缩放 rank 轴会同时收窄下面的统计表**（表格设为跟随缩放时），
**缩放时延轴不会** —— 它只裁剪画面，是放大镜，不是筛选器。

| 操作 | 效果 |
|---|---|
| 拖拽 | 平移（横纵皆可） |
| `Shift` + 拖拽 | 框选缩放：一个框同时定 rank 区间和时延区间 |
...
时延轴一旦被缩放就**锁成一个固定的纳秒区间**，之后平移 rank 轴时刻度不再自动贴合，
所以两个不同 rank 区间的柱高可以直接目测比较。跑出这个区间的柱子会在对应边缘标一个
小三角（▲/▼），免得被误读成"这条请求就这么高"。把时延轴一路缩小到重新包住全部数据，
它会自动回到逐窗自适应。
