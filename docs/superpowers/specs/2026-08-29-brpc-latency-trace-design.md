...
**`ts[i]` 存的是「偏移 + 1」，`0` 唯一地表示「未打点」。** 若直接存偏移，`0` 会同时表示两件事：未打点，以及「恰好落在 `base_counter` 上」。后者不是理论可能性 —— 客户端的 `AllocSlot` 与 C01 是相邻语句，在 100 MHz（10 ns/tick）的计数器下二者极可能落在同一个 tick 内，`ts[C01]` 因而天然为 0。两义合一会让 §10.1 的「点位非零」判据对一次完全正常的采集误报。`merge.py` 读取时减 1 还原。
